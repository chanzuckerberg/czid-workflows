...
def format_output_files(output_json_path, run):
    if output_json_path.startswith("s3://"):
        ...
        output_json = {}
        for stage in stages:
            output_json.update(format_output_files_s3(f"{output_json_path.rstrip('/')}/{stage}_output.json", run))
    ...
    return {
        ...
        f"step_counts_{run}": [v for k, v in output_json.items() if k.endswith("_count") and v],
    }

def format_output_files_s3(output_json_path, run):
    output_file = json.load(s3object(output_json_path).get()["Body"])
    return {"czid_short_read_mngs." + k.lstrip("czid_"): v for k, v in output_file.items() } #  reformat keys based on what's expected from local runs
