...
nonmatch_pattern = re.compile("NM:i:(\d+);")
cigar_pattern = re.compile("cg:Z:([A-Za-z0-9]+)")
...
class QualityCalculations:
    ...
    def calc_bitscore(self, alen, nonmatch):
        ...
        return (score*self._lambda - math.log(self._k))/math.log(2.)
...
def main(paf_file):
    ...
    df = pd.read_csv(paf_file, delimiter="\t", header=None, names=["qname", "qlen", "qstart", "qend",
        "strand", "tname", "tlen", "tstart", "tend", "nmatch", "alen", "mapq", "other"])
    df["nonmatch"] = df.other.map(lambda x: int(re.search(nonmatch_pattern, x).group(1)))
    qc = QualityCalculations()
    df["gap_openings"] = df.other.map(lambda x: qc.calc_gap_openings(re.search(cigar_pattern, x).group(1)))
    df["bitscore"] = [qc.calc_bitscore(a, n) for a, n in zip(df["alen"], df["nonmatch"])]
    df["evalue"] = [qc.calc_evalue(a, n) for a, n in zip(df["alen"], df["nonmatch"])]
    df["percent_ident"] = [(nmatch/a) * 100 for a, nmatch in zip(df["alen"], df["nmatch"])]
    df = df.round({"bitscore": 3, "percent_ident": 3}) 
    blast = df.loc[:, ["qname", "tname", "percent_ident", "alen", "nonmatch", "gap_openings", "qstart", "qend", "tstart", "tend", "evalue", "bitscore"]]
