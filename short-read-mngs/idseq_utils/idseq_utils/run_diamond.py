...
def _run_batch_job(job_name, job_queue, job_definition, environment, chunk_id, retries):
    ...
    _log_alignment_batch_job_status(job_id, job_queue, job_definition, chunk_id, 'SUBMITTED')

    delay = 60 + random.randint(-60 // 2, 60 // 2)  # Add some noise to de-synchronize chunks
    status = "SUBMITTED"
    ...
    while True:
        try:
            ...
            if e.response['Error']['Code'] == "TooManyRequestsException":
                ...
                log.error("unexpected_client_error_while_polling_job_status", extra={"job_id": job_id})
                raise e

        if status == "SUCCEEDED":
            _log_alignment_batch_job_status(job_id, job_queue, job_definition, chunk_id, status)
            return job_id
        if status == "FAILED":
            log.error("alignment_batch_job_failed", extra={'job_id': job_id, 'chunk_id': chunk_id, 'alignment_algorithm': ALIGNMENT_ALGORITHM})
            _log_alignment_batch_job_status(job_id, job_queue, job_definition, chunk_id, status)
...
def run_diamond(input_dir: str, chunk_dir: str, db_path: str, result_path: str, *queries: str):
