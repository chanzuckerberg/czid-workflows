...
from idseq_utils.trace_lock import TraceLock
import idseq_utils.command_patterns as command_patterns
...
def execute(command: Union[command_patterns.CommandPattern, str],
    ...
    if not isinstance(command, command_patterns.CommandPattern):
        ...
        log.write(warning=True, message="Command parameter is using legacy type str. Use idseq_utils.command_patterns.", obj_data={"cmd": command, "type": type(command)})
