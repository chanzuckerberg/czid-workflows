...
def diamond_blastx(cwd: str, par_tmpdir: str, block_size: float, database: str, out: str, queries: Iterable[str], chunk = False, join_chunks = 0):
    cmd = [
        "diamond", "blastx",
        "--multiprocessing",
        "--parallel-tmpdir", par_tmpdir,
...
def blastx_chunk(db_chunk: str, output_dir: str, *query: str):
    ...
    with TemporaryDirectory() as tmp_dir:
        ...
        shutil.copy(join(tmp_dir, "par-tmp", ref_block_name), join(output_dir, ref_block_name))
        shutil.copy(join(tmp_dir, "par-tmp", ref_dict_name), join(output_dir, ref_dict_name))
