...
def main():
    parser = argparse.ArgumentParser(description='Generate taxon count JSON from m8 and hitsummary files.')

    parser.add_argument("-f", '--m8-file', type=str, help='Path to deduped m8 file', required=True)
    parser.add_argument("-s", '--hitsummary', type=str, help='Path to hitsummary file', required=True)
    parser.add_argument("-c", '--count-type', type=str, choices=['NT', 'NR', 'merged_NT_NR'], help='Count type', required=True)
    parser.add_argument("-l", "--lineage", type=str, help='Path to lineage map file', required=True)
    parser.add_argument("--output", type=str, default="output", help="Output prefix for JSON file")
    parser.add_argument('--deuterostome-path', default=None, type=str, help='Path to deuterostome file')
    parser.add_argument('--taxon-whitelist-path', default=None, type=str, help='Path to taxon whitelist file')
    parser.add_argument('--taxon-blacklist-path', default=None, type=str, help='Path to taxon blacklist file')
    parser.add_argument('--duplicate-cluster-sizes-path', default=None, type=str, help='Path to duplicate cluster sizes file')
    args = parser.parse_args()

    suffix = "_with_dcr" if args.duplicate_cluster_sizes_path else ""

    generate_taxon_count_json_from_m8(
        args.m8_file, args.hitsummary, args.count_type, args.lineage,
        args.deuterostome_path, args.taxon_whitelist_path, args.taxon_blacklist_path,
        args.duplicate_cluster_sizes_path, f"{args.output}_counts{suffix}.json")
