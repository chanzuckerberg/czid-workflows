...
def _calc_contig2coverage(bam_filename):
            ...
            with open(output_csv_filenames[slice_idx], "w") as output_csv, \
                ...
                for contig_idx, contig_name in enumerate(input_bam.references):
                    if contig_idx % num_slices == slice_idx:
                        _process_contig(input_bam, output_csv, output_json, contig_name)
...
def generate_coverage_stats(
    ...
    coverage_summary_csv_output: str,
):
    # TODO: (tmorse) it seems weird to require the contigs file just for this check
    #   maybe try to do this with the sam file directly?
    if os.path.getsize(contigs_fasta) < MIN_CONTIG_FILE_SIZE:
        command.write_text_to_file('{}', coverage_json_output)
        command.write_text_to_file('No Contigs', coverage_summary_csv_output)
        return
    ...
    # run coverage info
    output_csv, output_json = _calc_contig2coverage(bam_file)
    os.rename(output_csv, coverage_summary_csv_output)
    os.rename(output_json, coverage_json_output)
...
class PipelineStepGenerateCoverageStats(PipelineStep):
    ...
    def run(self):
        """
          ...
          2. run pile up
        """
        contigs, _, read_contig_sam, _ = self.input_files_local[0]
        coverage_json, coverage_summary_csv = self.output_files_local()
        generate_coverage_stats(contigs, read_contig_sam, coverage_json, coverage_summary_csv)
